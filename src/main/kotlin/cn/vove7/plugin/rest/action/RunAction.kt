...
class RunAction @JvmOverloads constructor(
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val requestModel = getRequestModel(restFile) ?: return
    ...
    private fun showEnvPopup(envConfig: EnvConfig, onChosen: (name: String) -> Unit) {
        ...
        JBPopupFactory.getInstance().createPopupChooserBuilder<String>(envWithUrl).setTitle("Choose env")
    ...
    private fun runWithConfig(
        ...
        ApplicationManager.getApplication().executeOnPooledThread a@{
            if (env != null) {//requestModel.hasUnfilledParam() == true
                if ("headers" in env) {
                    requestModel.headers.putAll(
                        (env.get("headers") as JsonObject).toStringMap().filter { it.key !in requestModel.headers })
                }
                env.remove("headers")
                requestModel.fill(env)
            }

            val requestEntity = requestModel.toRestFileContent(project).lines().filter { it.isNotEmpty() }
                .joinToString("") { if (it.startsWith("#")) "$it\n" else "# $it\n" }

            var targetFileLazy: Lazy<File>? = null
            try {
                val startTime = System.currentTimeMillis()
                writeResponse(
                    project, document, restFile, """
                    |$requestEntity
                ...
                if (response.isUnText()) {//> 3M
                    ...
                    writeResponse(
                        project, document, restFile, "$requestEntity " +
                                "# Reading response..."
                    )
    ...
    private fun getRequestModel(restFile: RestFile): RequestModel? {
        val req = restFile.request ?: return null
    ...
    private fun createTmpRestFile(project: Project, requestContent: String): RestFile {
        return WriteCommandAction.runWriteCommandAction<RestFile>(project) {
            val psiFile: PsiFile = PsiFileFactoryImpl.getInstance(project)
                .createFileFromText("virtual", RestLanguage, requestContent)
