...
//    abstract fun isRetrofitMethod(ele: PsiElement): Boolean

//    abstract fun markElement(ele: PsiElement): PsiElement
    ...
    override fun getLineMarkerInfo(ele: PsiElement): LineMarkerInfo<*>? {
        ...
        if (target.isRetrofitMethod()) {
            ...
            return LineMarkerInfo(markElement, markElement.textRange, icon,
                { "Run Request" },
    ...
    private fun PsiMethod.apiMethodToRequestModel(): RequestModel? {
        ...
        return RequestModel().apply {
            ...
            url = urlValue.let {
                if (it.startsWith("http")) it else "{BASE_URL}${
                    it.let {
    ...
    private fun runRetrofitApi(e: MouseEvent, element: PsiElement) {
        ...
        if (targetFile.exists().not()) {
            ...
            JBPopupFactory.getInstance().createPopupChooserBuilder<String>(
                listOf(
    ...
    protected val supportAnnotationNames by lazy {
        arrayOf(
            "retrofit2.http.GET",
            "retrofit2.http.POST",
